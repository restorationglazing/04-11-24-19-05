...
const CaloriePreferences: React.FC<Props> = ({ onClose, onSave }) => {
  const [dailyTotal, setDailyTotal] = useState(DEFAULT_PREFERENCES.dailyTotal);
  const [distribution, setDistribution] = useState(DEFAULT_PREFERENCES.distribution);
    ...
    const loadPreferences = async () => {
      ...
      try {
        ...
        if (userData?.preferences?.caloriePreferences) {
          const prefs = userData.preferences.caloriePreferences;
          setDailyTotal(prefs.dailyTotal);
          
          // Calculate distribution percentages
          const total = prefs.dailyTotal;
          setDistribution({
            breakfast: Math.round((prefs.breakfast / total) * 100),
            lunch: Math.round((prefs.lunch / total) * 100),
            dinner: Math.round((prefs.dinner / total) * 100),
            snacks: Math.round((prefs.snacks / total) * 100)
          });
        }
      } catch (error) {
