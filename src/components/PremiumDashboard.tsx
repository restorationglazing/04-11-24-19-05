...
import { ChefHat, Calendar, BookOpen, Wand2, User, Activity } from 'lucide-react';
import confetti from 'canvas-confetti';
import { handleSuccessfulPayment } from '../utils/stripe';
import { auth, getUserData } from '../utils/firebase';
import { addPremiumUser, verifyPremiumStatus } from '../services/premium';
import PremiumUserBanner from './PremiumUserBanner';
import CaloriePreferences from './CaloriePreferences';
...
const PremiumDashboard: React.FC = () => {
  ...
  const [showCaloriePreferences, setShowCaloriePreferences] = useState(false);
      ...
      if (sessionId && auth.currentUser) {
        ...
        try {
          ...
          if (auth.currentUser.email) {
            await addPremiumUser(auth.currentUser.email, sessionId);
          }
    ...
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-8">
          <button
            onClick={() => setShowCaloriePreferences(true)}
            className="inline-flex items-center gap-2 px-6 py-3 bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200 transition-all"
          >
      ...
      {showCaloriePreferences && (
        <CaloriePreferences
          onClose={() => setShowCaloriePreferences(false)}
          onSave={() => {
            if (auth.currentUser) {
              getUserData(auth.currentUser.uid, true);
            }
          }}
        />
      )}
