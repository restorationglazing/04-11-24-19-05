...
export const addPremiumUser = async (email: string, sessionId?: string) => {
  if (!email) {
  ...
  try {
    ...
    throw new Error(error instanceof Error ? error.message : 'Failed to add premium user');
  }
};
...
export const verifyPremiumStatus = async (userId: string) => {
  ...
  try {
    ...
    throw new Error(error instanceof Error ? error.message : 'Failed to verify premium status');
  }
};